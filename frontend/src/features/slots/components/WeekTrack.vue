...
const props = withDefaults(
  ...
  { selectedId: null, showEmptyDays: false, selectable: true, groupLabel: undefined },
)
...
function lanesFor(day: number): Slot[][] {
  ...
  for (const slot of slotsFor(day)) {
    const lane = lanes.find(
      (l) => toMinutes(l[l.length - 1].endTime) <= toMinutes(slot.startTime),
    )
...
const rows = computed(() =>
  DAY_INDEXES.map((day) => ({ day, slots: slotsFor(day), lanes: lanesFor(day) })).filter(
    (row) => props.showEmptyDays || row.slots.length > 0,
  ),
)
...
function label(slot: Slot): string {
  return `${t(`common.days.${slot.dayOfWeek}`)}, ${shortTime(slot.startTime)}–${shortTime(
    slot.endTime,
...
<template>
  <div :role="selectable ? 'radiogroup' : undefined" :aria-label="selectable ? groupLabel : undefined">
    ...
    <div class="sm:hidden">
      ...
      <div
        v-for="row in rows"
        :key="row.day"
        class="border-t border-line/60 py-3"
      >
        <p class="u-label mb-2 text-ink-soft">{{ t(`common.days.${row.day}`) }}</p>
        ...
        <div v-else class="space-y-2">
          <component
            ...
            :class="
              selectedId === slot.id ? 'bg-brand text-surface' : 'bg-brand-soft text-brand'
            "
